...
import com.webclues.IPPSEngineer.activity.LoginActivity
import com.webclues.IPPSEngineer.view.MaterialAutoCompleteTextView
import com.webclues.IPPSEngineer.view.MaterialEditText
import java.io.File
...
class Utility {
    companion object {
        ...
        fun counter(leftTimeInMilliseconds: Long): String? {
            var simpledateformat: SimpleDateFormat = SimpleDateFormat(
                Content.COUNT_DOWN_TIMER,
                Locale.ENGLISH
            )
            var date: Date = Date(leftTimeInMilliseconds)
            var datetext: String = simpledateformat.format(date)
            return datetext
        }
...
//            editText.requestFocus()
    ...
    fun isPasswordValid(context: Context, editText: MaterialEditText): Boolean {      //Password valid
