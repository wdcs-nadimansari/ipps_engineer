...
import com.webclues.IPPSEngineer.adapter.NewNotificationsAdapter
...
class NewNotificationsFragment : Fragment() {
    ...
    private fun initview(view: View) {
        ...
        Content.JOB_TYPE = Content.NOTIFICATION
  ...
  /*  private fun loadfirstpage() {         //load first page in Notificationlist
                ...
                if (statuscode == 200) {
                    if (jsonObject.optBoolean("status")) {
                        ...
                        if (list != null && list.length() > 0) {
                            ...
                            notificationarraylist.addAll(Gson().fromJson(list.toString(), Array<NotificationModel>::class.java).toList())
    ...
    private fun loadNextPage(currentpage: Int) {               //load next page in Notifications list
        val call: Call<JsonObject> = apiInterface.notificationslist(currentpage)
        call.enqueue(object : Callback<JsonObject> {
